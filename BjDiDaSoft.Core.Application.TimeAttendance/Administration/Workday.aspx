<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Workday.aspx.cs" Inherits="BjDiDaSoft.Core.Application.TimeAttendance.Administration.Workday" %>
...
<head runat="server">
  ...
  <script type="text/javascript">
      ...
      var oWorkday = {
          ...
          CompanyId: null,
          UserId: null,
          WorkdayDescription: null,
          WorkdayShortName: null,
          WorkdayStatus: null,
          Operation: null
      };
      ...
      function setWorkday(operation) {
          oWorkday.Id = $("#workdayId").val();
          oWorkday.CompanyId = $("#companyId").val();
          oWorkday.UserId = $("#userId").val();
          oWorkday.Operation = operation;
          oWorkday.WorkdayDescription = $("#descripcion").val();
          oWorkday.WorkdayShortName = $("#abreviatura").val();

          if ($("#chkStatus").is(':checked')) {
              oWorkday.WorkdayStatus = "A";
          } else {
              oWorkday.WorkdayStatus = "I";
          }

          return oWorkday;
      };

      function saveWorkday(operation) {
          ...
          $.ajax({
              ...
              url: 'Services/ProcessData.asmx/SaveWorkday',
              data: "{'workdayModel':" + JSON.stringify(setWorkday(operation)) + ' }',
              dataType: 'json',
              async: true,
              success: function (data, textStatus) {
                  var message = JSON.parse(data.d);
                  if (message.ErrorCode == "0") {
                      $("#workdayMessage").text(message.ErrorMessage);

                      $("#dlgSaveWorkday").dialog({
                          modal: true,
                          buttons: {
      ...
      function deleteWorkday(operation) {
          ...
          if (rowidd != null) {
              for (var i in rowidd) {
                  ...
                  $.ajax({
                      type: 'POST',
                      contentType: "application/json; charset=utf-8",
                      url: 'Services/ProcessData.asmx/DeleteWorkday',
                      data: "{'workdayId':" + rowid.WORKDAY_ID + ",'companyId':" + rowid.COMPANY_ID + ",'userId':" + $("#userId").val() + ' }',
                      dataType: 'json',
                      async: true,
                      success: function (data, textStatus) {
                          var message = JSON.parse(data.d);
      ...
      function validateForm(operation) {
          // Abreviatura jornada laboral
          if ($("#abreviatura").val() == "") {
              $("#dlgMessage").dialog();
              $("#textMessage").text("Ingresar abreviatura jornada laboral");
              $("#dlgMessage").dialog("open");
              return false;
          }
          // Descripción jornada laboral
          if ($("#descripcion").val() == "") {
              $("#dlgMessage").dialog();
              $("#textMessage").text("Ingresar descripción jornada laboral");
              $("#dlgMessage").dialog("open");
              return false;
          }
      ...
      jQuery(document).ready(function () {
          ...
          //acordeones
          $("#accWorkdayA").accordion({ collapsible: true, autoHeight: false });
          $("#accWorkdayB").accordion({ collapsible: true, autoHeight: false });

          $("#descripcion").attr("maxlength", 80);
          $("#abreviatura").attr("maxlength", 2);
          ...
          //grid jornada laboral
          $.ajax({
              dataType: "json",
              type: "post",
              url: "Services/LoadColumns.asmx/GetWorkdayColumns",
              data: "{}",
              contentType: "application/json;",
              async: false, //esto es requerido, de otra forma el jqgrid se cargaria antes que el grid
              success: function (data) {
                  var workdays = JSON.parse(data.d);
                  $.each(workdays, function (index, workday) {
                      colMode.push({ name: workday.Name, index: workday.index, label: workday.label, width: workday.width, align: workday.align, editable: workday.editable, edittype: workday.editType, editrules: { edithidden: true }, hidden: workday.hidden });
                  })
          ...
          $("#grid").jqGrid(
          {
              datatype: function () {
                  $.ajax(
                  {
                      url: "Services/LoadData.asmx/GetWorkdays", //PageMethod
              ...
              jsonReader: //Set the jsonReader to the JQGridJSonResponse squema to bind the data.
              {
                  ...
                  repeatitems: true,
                  cell: "Row",
                  id: "WORKDAY_ID" //index of the column with the PK in it    
              },
              ...
              multiselect: true,
              sortname: "WORKDAY_DESCRIPTION", //Default SortColumn
          ...
          .navButtonAdd('#pager', {
              caption: "", //Add
              title: "Agregar nueva jornada laboral",
              ...
              onClickButton: function () {
                  ...
                  $("#dialog").dialog({
                      ...
                      draggable: true,
                      resizable: false,
                      title: "Ingresar Jornada Laboral",
                      modal: true,
                      buttons: {
          ...
          .navButtonAdd('#pager', {
              ...
              onClickButton: function () {
                  ...
                  if (rowidd != null) {
                      ...
                      $("#workdayId").val(rowid.WORKDAY_ID);
                      $("#abreviatura").val(rowid.WORKDAY_SHORT_NAME);
                      $("#descripcion").val(rowid.WORKDAY_DESCRIPTION);

                      if (rowid.WORKDAY_STATUS == "A") {
...
<body>
    ...
    <div>
      ...
      <div id="dialog" title="Insertar Jornada Laboral">
       <div id="accWorkdayA" >
        <h3>
         <a style="color: Black"><b>Datos Jornada Laboral</b></a>
        </h3>
        <table>
         <tr>
          <td><label for="abreviatura">Abreviatura:</label></td>
          <td><input id="abreviatura" name="abreviatura" required="required" type="text" placeholder="Abreviatura" runat="server" style="width:35px;" />
              <input type="hidden" id="companyId" name="companyId" runat="server" />
              <input type="hidden" id="workdayId" name="workdayId" runat="server" />
              <input type="hidden" id="userId" name="userId" runat="server" />
          </td>
         </tr>
         <tr>
          <td><label for="descripcion">Descripci&oacute;n:</label></td>
          <td><input id="descripcion" name="descripcion" required="required" type="text" placeholder="Descripci&oacute;n" runat="server" style="width:400px;" /></td>
         </tr>
         <tr>
      ...
      <div id="dlgSaveWorkday" title="Grabar Jornada Laboral">
	   <p>
        <span class="ui-icon ui-icon-circle-check" style="float:left; margin:0 7px 50px 0;"></span>
        <b id="workdayMessage"></b>
	   </p>
      </div>
