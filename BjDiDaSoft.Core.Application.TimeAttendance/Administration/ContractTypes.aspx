<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="ContractTypes.aspx.cs" Inherits="BjDiDaSoft.Core.Application.TimeAttendance.Administration.ContractTypes" %>
...
<head runat="server">
  ...
  <script type="text/javascript">
      ...
      function setContractType(operation) {
          oContractType.Id = $("#codigo").val();
          oContractType.CompanyId = $("#companyId").val();
          oContractType.UserId = $("#userId").val();
          oContractType.Operation = operation;
          oContractType.ContractTypeDescription = $("#descripcion").val();

          if ($("#chkStatus").is(':checked')) {
              oContractType.ContractTypeStatus = "A";
          } else {
              oContractType.ContractTypeStatus = "I";
          }

          return oContractType;
      };

      function saveContractType(operation) {
          ...
          $.ajax({
              ...
              url: 'Services/ProcessData.asmx/SaveContractType',
              data: "{'contractTypeModel':" + JSON.stringify(setContractType(operation)) + ' }',
              dataType: 'json',
              async: true,
              success: function (data, textStatus) {
                  var message = JSON.parse(data.d);
                  if (message.ErrorCode == "0") {
                      $("#contractTypeMessage").text(message.ErrorMessage);

                      $("#dlgSaveContractType").dialog({
                          modal: true,
                          buttons: {
      ...
      function deleteContractType(operation) {
          ...
          if (rowidd != null) {
              for (var i in rowidd) {
                  ...
                  $.ajax({
                      type: 'POST',
                      contentType: "application/json; charset=utf-8",
                      url: 'Services/ProcessData.asmx/DeleteContractType',
                      data: "{'contractTypeId':" + rowid.CONTRACT_TYPE_ID + ",'companyId':" + rowid.COMPANY_ID + ",'userId':" + $("#userId").val() + ' }',
                      dataType: 'json',
                      async: true,
                      success: function (data, textStatus) {
                          var message = JSON.parse(data.d);
      ...
      function validateForm(operation) {
          ...
          if ($("#codigo").val() == "") {
              $("#dlgMessage").dialog();
              $("#textMessage").text("Ingresar código tipo contrato");
              $("#dlgMessage").dialog("open");
              return false;
          }
          // Descripción tipo contrato
          if ($("#descripcion").val() == "") {
              $("#dlgMessage").dialog();
              $("#textMessage").text("Ingresar descripción tipo contrato");
              $("#dlgMessage").dialog("open");
              return false;
          }
      ...
      jQuery(document).ready(function () {
          ...
          //acordeones
          $("#accContractTypeA").accordion({ collapsible: true, autoHeight: false });
          $("#accContractTypeB").accordion({ collapsible: true, autoHeight: false });
          ...
          $.ajax({
              dataType: "json",
              type: "post",
              url: "Services/LoadColumns.asmx/GetContractTypeColumns",
              data: "{}",
              contentType: "application/json;",
              async: false, //esto es requerido, de otra forma el jqgrid se cargaria antes que el grid
              success: function (data) {
                  var contractTypes = JSON.parse(data.d);
                  $.each(contractTypes, function (index, contractType) {
                      colMode.push({ name: contractType.Name, index: contractType.index, label: contractType.label, width: contractType.width, align: contractType.align, editable: contractType.editable, edittype: contractType.editType, editrules: { edithidden: true }, hidden: contractType.hidden });
                  })
          ...
          $("#grid").jqGrid(
          {
              datatype: function () {
                  $.ajax(
                  {
                      url: "Services/LoadData.asmx/GetContractTypes", //PageMethod
              ...
              jsonReader: //Set the jsonReader to the JQGridJSonResponse squema to bind the data.
              {
                  ...
                  repeatitems: true,
                  cell: "Row",
                  id: "CONTRACT_TYPE_ID" //index of the column with the PK in it    
              },
              ...
              multiselect: true,
              sortname: "CONTRACT_TYPE_DESCRIPTION", //Default SortColumn
          ...
          .navButtonAdd('#pager', {
              caption: "", //Add
              title: "Agregar nuevo tipo contrato",
          ...
          .navButtonAdd('#pager', {
              ...
              onClickButton: function () {
                  ...
                  if (rowidd != null) {
                      ...
                      $("#codigo").val(rowid.CONTRACT_TYPE_ID);
                      $("#descripcion").val(rowid.CONTRACT_TYPE_DESCRIPTION);

                      if (rowid.CONTRACT_TYPE_STATUS == "A") {
...
<body>
    ...
    <div>
      ...
      <div id="dialog" title="Insertar Tipo Contrato">
       <div id="accContractTypeA" >
        <h3>
         <a style="color: Black"><b>Datos Tipo Contrato</b></a>
        </h3>
        <table>
         <tr>
      ...
      <div id="dlgSaveContractType" title="Grabar Tipo Contrato">
	   <p>
        <span class="ui-icon ui-icon-circle-check" style="float:left; margin:0 7px 50px 0;"></span>
        <b id="contractTypeMessage"></b>
	   </p>
      </div>
